...
export const Button = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "relative px-3 py-1 text-base font-medium rounded-[8px] transition-colors disabled:bg-slate-300 disabled:grayscale disabled:pointer-events-none",
        className,
        cName(variant),
      )}
      disabled={disabled}
...
export const ButtonRefresh: React.FC<Partial<ButtonProps>> = ({
  ...
  return (
    <div className={cn("w-10 h-full", className)}>
      <button type="button" className="size-full" onClick={onClick}>
        {/* <Image
          src={iconRefresh}
          alt="refresh"
          width={0}
