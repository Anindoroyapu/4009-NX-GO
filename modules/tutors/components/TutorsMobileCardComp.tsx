...
const TutorsMobileCardComp: FC<TeacherCardProps> = ({
  image,
  name,
  university,
  subjects,
  ...
  // const { instituteList } = useTutorsData();
  // const TutorInstituteLogo = Object.values(
  //   instituteList as Record<string, { sl: number; institute_logo: string }>,
  // ).find((edu: any) => edu.institute_name === university)?.institute_logo;
  ...
  return (
    <div className="p-5 bg-white rounded-[20px] border border-[#021713]/10 flex flex-col items-center justify-center   gap-[18px]">
      <div className="w-full flex items-center gap-3.75">
        <div className="w-15 h-15 bg-center bg-cover">
          <Image
            src={
              universityLogo
        ...
        <div className="flex flex-col w-[calc(100%-80px)] lg:items-center gap-[5px]">
          <h3 className="text-[22px] leading-6.25 font-bold text-[#021713] text-start lg:text-center">
            {name}
          </h3>
          <p className="text-[14px] leading-[18px] text-[#02171399] text-start lg:text-center">
            {university}
          </p>
        </div>
      </div>
      ...
      <div className="w-full flex flex-col  gap-[18px]">
        <div className="flex items-center text-start gap-3">
          ...
          <p className="text-[16px] text-start font-bold text-black">
            {subjects}
          </p>
        </div>
        ...
        <div className="flex gap-[9px] w-full">
          <Link
            ...
            className="w-full  text-center py-2 text-nowrap h-10 border border-[#021713] rounded-[8px] text-[16px] font-bold uppercase text-[#021713] hover:bg-[#021713] hover:text-[#72FF31] cursor-pointer"
          >
            প্রোফাইল দেখুন
          </Link>
          <button
            onClick={() => openModal("contactTutor", { profile_sl, name })}
            className=" w-full text-center py-2 text-nowrap h-10 bg-[#72FF31] rounded-[8px] text-[16px] font-bold uppercase text-[#021713] hover:bg-[#021713] hover:text-[#72FF31] cursor-pointer"
          >
            যোগাযোগ করুন
          </button>
        </div>
...
export default TutorsMobileCardComp;
